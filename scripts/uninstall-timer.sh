#!/usr/bin/env bash
# Remove the vdb background indexer and label-extraction pass. Does not touch
# the index database or ~/.claude — only the systemd --user units this
# installed.
set -euo pipefail
...
if command -v systemctl >/dev/null 2>&1; then
    systemctl --user disable --now vdb-index.timer vdb-label.timer 2>/dev/null || true
fi
rm -f "${UNIT_DIR}/vdb-index.service" "${UNIT_DIR}/vdb-index.timer" \
      "${UNIT_DIR}/vdb-label.service" "${UNIT_DIR}/vdb-label.timer"
if command -v systemctl >/dev/null 2>&1; then
    systemctl --user daemon-reload
fi

echo "removed: ${UNIT_DIR}/vdb-{index,label}.{service,timer}"
